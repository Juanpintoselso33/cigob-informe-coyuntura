...
export const FORMULAS: Record<string, Formula> = {
  ...
  desequilibrio_monetario: {
    ...
    leyenda: "El indicador cruza dos componentes en vez de promediarlos. A mide, sobre el total de la liquidez privada (pesos más los dólares depositados, valuados en pesos), qué proporción sigue en pesos de uso transaccional: es la composición de la liquidez que queda depositada. B mide la compra neta de divisas del sector privado no financiero en el mercado de cambios: es presión compradora, y no dice adónde fue ese dinero — el BCRA estimó que cerca del 80% quedó depositado localmente. Aparece aunque no toque ningún depósito. Cada uno se lleva a una posición de 0 a 1 interpolando entre los percentiles de su ventana de calibración (A desde enero de 2021, que es cuando el BCRA empieza a publicar el M2 transaccional privado; B desde abril de 2025, la apertura del cepo a personas humanas, para no mezclar regímenes cambiarios). La tensión sale de interpolar bilinealmente entre las cuatro esquinas de la matriz: confianza real 0, dolarización contenida en el sistema 40, fuga oculta fuera del sistema 77,5 y deterioro dentro y fuera 90. La matriz no es simétrica a propósito: que se degrade la fuga cuesta casi el doble que se degrade el stock, porque la fuga fuera del sistema es la señal grave. El puntaje del ITCM es el complemento de la tensión.",
  },
  recaudacion: {
