...
export const FICHAS: Record<string, Ficha> = {
  ...
  itcm: {
    ...
    validacion: [
      ...
      "Desde julio de 2026 se agrega un segundo contraste externo, independiente del mercado: el Índice Líder de la Universidad Torcuato Di Tella, que resume la marcha de la actividad económica. Cubre justamente la debilidad del primero, porque mantiene el co-movimiento en los cambios mes a mes, donde la comparación con el riesgo país se desvanece. Con una salvedad que se publica junto al número: el orden temporal va al revés de lo que sugiere el nombre del índice externo — el ajuste mejora cuando se adelanta el ITCM y empeora cuando se adelanta el líder, así que sirve para validar el mismo mes y no como alerta temprana. El indicador integraba el cinturón de vida cotidiana y se movió acá porque mide el ciclo de la actividad y no una condición de los hogares.",
