...
def test_homicidios_sobrevive_aunque_sea_bajisimo_volumen():
    """Control positivo Y negativo en la misma corrida: seis categorías con
    ...
    filas = [
        ...
        ("2025", "Hurtos", 308523),
        ("2025", "Otros delitos contra la propiedad", 249754),
...
def test_una_categoria_irrelevante_no_entra_aunque_tenga_mucho_volumen():
    ...
    filas = [
        ("2025", "Contravenciones", 999_999_999),  # no está en TIPOS_RELEVANTES
        ("2025", "Homicidios dolosos", 1613),
    ]
    out = snic._parse_snic_csv(_csv(filas))
...
def test_falta_un_tipo_relevante_no_revienta_la_corrida():
    """Si el CSV de un año no trae alguna de las categorías esperadas
    (cambio de nomenclatura de la fuente), el colector sigue con las que sí
    están en vez de reventar — sólo loguea la ausencia."""
